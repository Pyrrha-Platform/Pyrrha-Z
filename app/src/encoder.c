...
static const struct json_obj_descr sensor_descr[] = {
    ...
    JSON_OBJ_DESCR_PRIM_NAMED(struct sensor_value, "frac", val2, JSON_TOK_NUMBER),
};
...
static const struct json_obj_descr rht_sensor_descr[] = {
    JSON_OBJ_DESCR_OBJECT(struct rht_data, temperature, sensor_descr),
    JSON_OBJ_DESCR_OBJECT(struct rht_data, humidity, sensor_descr),
};
...
int sensor_data_encode(struct pyrrha_data * data, char * buffer, size_t buffer_len){
    ...
    /* Null terminate the buffer in case the json library doesn't*/
    buffer[len] = '\0';
